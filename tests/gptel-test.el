    ...
    (unwind-protect
        (cl-letf (((symbol-function 'imoogi-gptel--api-key) (lambda () "key"))
                  ...
                  ((symbol-function 'network-lookup-address-info)
                   (lambda (&rest _) '([127 0 0 1])))
                  ((symbol-function 'open-network-stream)
                   (lambda (_name _buffer _host _port &rest parameters)
                     (let ((type (plist-get parameters :type)))
                       (push type probe-types)
                       (if (eq type 'tls)
                           (list 'fake-process :type 'tls)
                         'fake-process))))
          ...
          (with-temp-buffer
            ...
            (should (search-forward "action=model-network-ca" nil t))
            (should (search-forward "action=model-network-dns" nil t))
            (should (search-forward ":result=ok" nil t))
            (should (search-forward "action=model-network-tcp" nil t))
            (should (search-forward "action=model-network-tls" nil t))))
      (delete-directory directory t))))

(ert-deftest imoogi-gptel-private-ca-is-added-to-gnutls-trustfiles ()
  (let* ((ca-file (make-temp-file "imoogi-private-ca" nil ".pem"))
         (imoogi-ca-certificate-file ca-file)
         (gnutls-trustfiles nil))
    (unwind-protect
        (progn
          (should (equal (imoogi-system-configure-ca-certificate) ca-file))
          (should (member ca-file gnutls-trustfiles)))
      (delete-file ca-file))))

(ert-deftest imoogi-gptel-private-ca-must-be-readable ()
  (let ((imoogi-ca-certificate-file
         (expand-file-name "missing-ca.pem" (make-temp-file "imoogi-ca" t))))
    (should-error (imoogi-system-configure-ca-certificate)
                  :type 'user-error)))

(ert-deftest imoogi-gptel-global-ca-command-persists-and-clears-setting ()
  (let* ((directory (make-temp-file "imoogi-gptel-ca-config" t))
         (ca-file (expand-file-name "company-ca.pem" directory))
         (imoogi-system-config-file (expand-file-name "system.json" directory))
         (imoogi-gptel-log-file (expand-file-name "gptel.log" directory))
         (imoogi-gptel-provider 'litellm)
         (imoogi-gptel-gateway-url "https://gateway.example.test")
         (imoogi-gptel-api-protocol 'openai-chat)
         (imoogi-gptel-endpoint "/v1/chat/completions")
         (imoogi-gptel-models-endpoint "/v1/models")
         (imoogi-gptel-models '(model-a))
         (imoogi-gptel-default-model 'model-a)
         (imoogi-gptel-litellm-profiles nil)
         (imoogi-gptel-active-profile nil)
         (imoogi-ca-certificate-file nil)
         (gnutls-trustfiles nil))
    (unwind-protect
        (progn
          (write-region "test-ca" nil ca-file nil 'silent)
          (imoogi-system-set-ca-certificate ca-file)
          (should (equal imoogi-ca-certificate-file ca-file))
          (should (file-readable-p imoogi-system-config-file))
          (should (member ca-file gnutls-trustfiles))
          (imoogi-system-clear-ca-certificate)
          (should-not imoogi-ca-certificate-file)
          (should-not (member ca-file gnutls-trustfiles)))
      (delete-directory directory t))))
