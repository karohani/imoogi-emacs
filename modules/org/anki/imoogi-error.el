...
(defconst imoogi-error-table
  '(("binary_not_found" .
    ...
    ("multiline_answer_missing" .
     "This heading asks for a multiline card -- ANKI_DIRECTION or ANKI_INCREMENTAL is on -- but imoogi found no list of answers in its body, so the heading was skipped and nothing was written for it. A multiline card takes its question from the heading title and its answers from the first list in the body, so the body needs one: write the answers as an Org list (`- one answer' per line, or a numbered list) directly under the heading. If this heading was not meant to be a multiline card, give ANKI_DIRECTION and ANKI_INCREMENTAL the value `nil' in its own drawer -- and note that either may have been inherited from an ancestor heading or a file-level `#+PROPERTY:' line rather than written here. A `#+BEGIN_EXTRA' block is supplementary content and is never read as the answers.")
