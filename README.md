...
한 번 `make build-org-preview`로 Go 서버를 빌드한다. Org 파일에서 **`C-c h o v`**
또는 `M-x imoogi-org-preview`를 실행하면 로컬 서버와 브라우저가 열린다.
저장하지 않은 수정도 마지막 변경 후 0.5초 뒤 갱신한다. **`C-c h o V`**로
현재 버퍼의 자동 미리보기를 종료한다.

왼쪽 **목차**와 본문 제목은 실제 Org 깊이에 따라 빨강·파랑·초록·노랑으로 표시한다.
...
위치 전송에는 Org 문법 해석을 수행하지 않는다. 전체 버퍼 복사와 JSON 인코딩은
Emacs에서 실행하므로 대용량 문서에서 비용이 완전히 없어지는 것은 아니다.
Org 문법 전체 및 정식 내보내기와의 완전한 일치는 보장하지 않는다.
